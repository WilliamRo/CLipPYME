...
class fitTask(taskDef.Task):
    def __init__(self, 
                 dataSourceID,              # .h5 fileName
                 index,                     # 0
                 threshold,
                 metadata, 
                 fitModule,
                 dataSourceModule='HDFDataSource',  # TQDataSource
                 bgindices = [], 
                 SNThreshold = False, 
                 driftEstInd=[], 
                 calObjThresh=200):
    ...
    def __call__(self, gui=False, taskQueue=None):
        ...
        self.data = cameraMaps.correctImage(md, self.data.squeeze())
        self.data = self.data.reshape((self.data.shape[0], 
                                       self.data.shape[1], 1))

        #calculate background　################
        self.bg = self.md['Camera.ADOffset']  ## self.bg = 0
        if not len(self.bgindices) == 0:
            self.bg = cameraMaps.correctImage(
                md, bufferManager.bBuffer.getBackground(self.bgindices)).reshape(
                    self.data.shape)
        ...
        #print '>> bgd.shape:', bgd.shape
        #print '>> self.calcThreshold().shape:', self.calcThreshold().shape
        ...
        self.ofd.FindObjects(self.calcThreshold(),   # thresholdFactor
                             0,                      # numThresholdSteps
                             splitter = sfunc,
