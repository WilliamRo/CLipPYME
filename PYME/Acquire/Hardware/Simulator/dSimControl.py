...
class dSimControl(wx.Panel):
    ...
    def _init_ctrls(self, prnt):
        ...
        #s416-import
        ######## Test Frames #########
        sbsizer = wx.StaticBoxSizer(wx.StaticBox(self, -1, 'Test Frames'), 
                                    wx.VERTICAL)

        self.testStatus = wx.StaticText(self, -1,
              label = 'No test frames imoprted\n\n')
        sbsizer.Add(self.testStatus, 0, wx.ALL|wx.EXPAND, 2)
        ...
        ## label
        self.lbFPS = wx.StaticText(self, -1,
              label = 'Theoritical FPS: ')
        hsizer.Add(self.lbFPS, 0, wx.ALIGN_CENTER_VERTICAL|wx.RIGHT, 2)
        #self.lbFPS.Show(False)
        ## textbox
        self.txtFPS = wx.TextCtrl(self, -1, value = '10')
        hsizer.Add(self.txtFPS, 0, wx.ALIGN_CENTER_VERTICAL|wx.RIGHT, 2)
        #self.txtFPS.Show(False)
        ## btn
        self.bSetFPS = wx.Button(self, -1, 'Set FPS')
        self.bSetFPS.Bind(wx.EVT_BUTTON, self.OnBSetFPS)
        hsizer.Add(self.bSetFPS, 0, wx.ALIGN_CENTER_VERTICAL|wx.RIGHT, 2)
        self.bSetFPS.Enable(False)

        ## btn
        self.bImport = wx.Button(self, -1, 'Import Test Frames')
        self.bImport.Bind(wx.EVT_BUTTON, self.OnBImportButton)
        hsizer.Add(self.bImport, 0, wx.ALIGN_CENTER_VERTICAL|wx.RIGHT, 2)
...
#        pylab.show()
    ...
    #s416-import
    def OnBImportButton(self, event):
        fn = wx.FileSelector('Read test frames from file', 
        ...
        import PYME.gohlke.tifffile as tifffile
        print '> Ready to set test frames...'
        frames = tifffile.TIFFfile(fn).asarray()

        self.testStatus.SetLabel('File: %s\nSize: %d x %d\nTotal Frames: %d' % 
                                 (fn, frames.shape[1], frames.shape[2], frames.shape[0]))

        pa = self.scope.pa
        pa.stop()
        pa.dsa = np.zeros([frames.shape[1], 
                            frames.shape[2], 
                            pa.GetSeqLength()], 
                            dtype = 'uint16', order = 'F')
        self.mainframe._refreshDataStack()
        pa.cam.setFrames(frames)

        pa.start()
        self.bSetFPS.Enable(True)
        #self.lbFPS.Show(True)
        #self.txtFPS.Show(True)
        #self.bSetFPS.Show(True)
        print '> Test frames set...'

    def OnBSetFPS(self, event):
        try:
            fps = float(self.txtFPS.GetValue())
        except:
            fps = 0

        if fps > 1000 or fps < 1:
            self.txtFPS.SetValue('10')
            return

        interval = 1.0 / fps

        self.scope.cam.compT.interval = interval
