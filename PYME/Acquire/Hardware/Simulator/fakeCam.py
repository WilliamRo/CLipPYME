...
#class compThread(processing.Process):
    ...
    def run(self):
        ...
        while not self.kill:
            ...
            #s416-import
            if 'testFrames' in dir(self):
                self.im = self.testFrames[self.testIndex, :, :].astype('uint16')
                self.testIndex += 1
                if self.testIndex == self.testFrames.shape[0]: 
                    self.testIndex = 0
                time.sleep(self.interval)
            elif not self.fluors == None and not 'spec' in self.fluors.fl.dtype.fields.keys():
                if self.biplane:
                    self.im = self.noiseMaker.noisify(
                        rend_im.simPalmImFBP(self.XVals, 
                                             self.YVals, 
                                             zPos,
                                             self.fluors, 
                                             laserPowers=self.laserPowers, 
                                             intTime=self.intTime, 
                                             deltaZ = self.deltaZ)
                        )[:,:].astype('uint16')
                else:
                    #self.im = self.noiseMaker.noisify(
                    #    rend_im.simPalmImFI(self.XVals + xp, 
                    #                        self.YVals + yp, 
                    #                        zPos,
                    #                        self.fluors, 
                    #                        laserPowers=self.laserPowers, 
                    #                        intTime=self.intTime, 
                    #                        position=[xp,yp,zPos], 
                    #                        illuminationFunction=self.illumFcn)
                    #    )[:,:].astype('uint16')
                     self.im = rend_im.simPalmImFI(self.XVals + xp, 
                                            self.YVals + yp, 
                                            zPos,
                                            self.fluors, 
                                            laserPowers=self.laserPowers, 
                                            intTime=self.intTime, 
                                            position=[xp,yp,zPos], 
                                            illuminationFunction=self.illumFcn)[:,:].astype('uint16')
            else:
                ...
                self.im = self.noiseMaker.noisify(
                    rend_im.simPalmImFI(self.XVals + xp, 
                                        ...
                                        position=[xp,yp,zPos], 
                                        illuminationFunction=self.illumFcn, 
                                        Chan2XOffset=70.*self.XVals.shape[0]/4., 
                                        Chan2ZOffset=self.deltaZ)
                    )[:,:].astype('uint16')
...
class FakeCamera:
    ...
    def __init__(self, XVals, YVals, noiseMaker, zPiezo, zOffset=50.0, fluors=None, laserPowers=[0,50], xpiezo=None, ypiezo=None, illumFcn = 'ConstIllum'):
        ...
        self.compT = compThread(self.XVals[self.ROIx[0]:self.ROIx[1]], 
                                self.YVals[self.ROIy[0]:self.ROIy[1]], 
                                self.zPiezo, 
                                self.zOffset,
                                self.fluors, 
                                self.noiseMaker, 
    ...
    def setFluors(self, fluors):
        ...
        self.compT = compThread(self.XVals[self.ROIx[0]:self.ROIx[1]], 
                                self.YVals[self.ROIy[0]:self.ROIy[1]], 
                                self.zPiezo, self.zOffset,
                                self.fluors, 
                                self.noiseMaker, 
                                laserPowers=self.compT.laserPowers, 
                                intTime=self.intTime, 
                                xpiezo=self.xPiezo, 
                                ypiezo=self.yPiezo, 
                                illumFcn=self.illumFcn)
        #s416-debug
        self.compT.laserPowers = fluors.fl['exc'][0,:]
    ...
    #s416-import -----------------------------------------------------------
    def bUseTestFrame(self):
        return 'testFrames' in dir(self)
    ...
    def setFrames(self, frames):
        ...
        self.XVals = 70 * scipy.arange(-frames.shape[1] / 2, frames.shape[1] / 2)
        self.YVals = 70 * scipy.arange(-frames.shape[2] / 2, frames.shape[2] / 2)
        self.compT = compThread(self.XVals, 
                                self.YVals, 
                                self.zPiezo, self.zOffset,
                                self.fluors, 
                                self.noiseMaker, 
        ...
        self.compT.testFrames = frames
        self.compT.testIndex = 0
        self.compT.interval = 0.1
        self.compT.start()

        self.compT.aqRunning = running
        print '> test frames imported.'
