...
class TaskQueueSet(Pyro.core.ObjBase):
    ...
    def getTasks(self, workerName='Unspecified'):
        ...
        if LOCAL and not compName in workerName:
            ...
            print '> LOCAL and not compName in workerName'
            print '> returning [] ...'
            return []
        ...
        with self.getTaskLock:
            ...
            if self.getNumberOpenTasks(exact=False) < 1:
                print '> self.getNumberOpenTasks(exact=False) < 1'
                print '> setting res to [] ...'
                res = []
            else:
                if not workerName in self.activeWorkers:
                    self.activeWorkers.append(workerName)
                ...
                res = queuesWithOpenTasks[
                    int(numpy.round(len(queuesWithOpenTasks)*numpy.random.rand() - 0.5))].getTasks(
                    self.activeWorkers.index(workerName), len(self.activeWorkers))
